...
// 8ewrh8hke kaluterh h ulopoihsh twn countingsort,mergesort,dataentry se ksexwrista .c kai .h files opote 8a prepei na uparxoun entos tou arxeiou ths askhshs
...
// Paromoiws me ta countingsort kai mergesort to effects.csv arxeio einai aparaithto na uparxei entos tou fakelou, egine allagh tou onomatos se effects.csv gia logous eukolias ston kwdika
int main() {
    ...
    // Orizoume mia metablhth maxValue h opoia 8a xreiastei sthn taksinomhsh
    int maxValue = entries[0].Value;
    for (int i = 1; i < entryCount; i++) {
        if (entries[i].Value > maxValue)
            maxValue = entries[i].Value;
    }
